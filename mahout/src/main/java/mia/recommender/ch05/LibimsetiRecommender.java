...
public class LibimsetiRecommender implements Recommender {
    ...
    public LibimsetiRecommender() throws TasteException, IOException {
        this(new FileDataModel(
                readResourceToTempFile("/ch05/ratings.dat")));
    }
    ...
    public LibimsetiRecommender(DataModel model)
        ...
        FastIDSet[] menWomen = GenderRescorer
                .parseMenWomen(readResourceToTempFile("/recommender/ch05/gender.dat"));
        men = menWomen[0];
        women = menWomen[1];
    ...
    static File readResourceToTempFile(String resourceName)
            throws IOException {
        String absoluteResource = resourceName.startsWith("/")
                ? resourceName : '/' + resourceName;
        InputSupplier<? extends InputStream> inSupplier;
        try {
            URL resourceURL = Resources.getResource(
                    LibimsetiRecommender.class, absoluteResource);
