...
public class QueryResource {
	...
	public Yijings getByPaging(@QueryParam("start") final int start,
		...
		final UriBuilder ub = uriInfo.getAbsolutePathBuilder().replacePath(
				"query-resource/yijing");
		...
		for (int i = 0; i < max; i++) {
			...
			final URI location = ub.clone()
					.queryParam("id", yijing.getSequence()).build();
			final Link link = new Link("detail", location.toASCIIString(),
